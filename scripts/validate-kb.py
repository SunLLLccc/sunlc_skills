...
注：skill 源码的通用化（grep dsp skills/ = 0）不在本脚本职责内——生成出来的 KB 文档
合法包含项目名（如项目叫 dsp，KB 里就有 dsp）。skill 源码通用化由 CI 单独 grep 校验。
...
def check_placeholders(report, kb_dir):
    ...
    刻意排除 06-校验报告.md——它是描述核对格式的元报告，正文 legend 永久含
    `{文件路径}:{行号}` 等格式 token（说明三元组结构），不是未填槽位。
    """
    docs = [d for d in _kb_docs(kb_dir) if d.name != "06-校验报告.md"]
    if not docs:
...
def check_required_sections(report, kb_dir):
    """4（WARN）：必填章节存在性。"""
    checks = []
    doc01 = kb_dir / "01-项目概览.md"
    if doc01.exists():
        t = doc01.read_text(encoding="utf-8")
        if "项目类型" not in t:
            checks.append(("01-项目概览 有'项目类型'章节", "未找到'项目类型'"))
        if "入口" not in t:
            checks.append(("01-项目概览 有'入口'相关章节", "未找到'入口'"))
    readme = kb_dir / "README.md"
    if readme.exists():
...
def _kb_docs(kb_dir):
    ...
    if not kb_dir.exists():
        return []
    docs = []
    for name in ("01-项目概览.md", "02-技术栈与架构.md", "03-开发规范.md",
                 "04-API索引.md", "05-构建打包部署.md", "06-校验报告.md", "README.md"):
        p = kb_dir / name
        if p.exists():
            docs.append(p)
    return docs
