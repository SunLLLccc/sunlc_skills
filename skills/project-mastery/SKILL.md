...
- `{PROJECT_ROOT}/docs/project-knowledge/06-校验报告.md`（波次 4，默认 `pm-verify-lite` 产；opt-in `pm-verify` 产全量版，文件名相同）
...
**波次间的硬依赖**（不可乱序）：
- 波次 2 全部依赖波次 1 的 `01-项目概览.md`（作为上下文输入）和 `.codebase/scan-result.json`（路由依据）。
...
**前置依赖**：入口判定已声明走【学习】。
**本波次产出**：`01-项目概览.md` + `.codebase/scan-result.json` + `.codebase/scan-summary.md`。
...
**本波次产出**：`06-校验报告.md`（lite 与 full 同名，内容详略不同）。
...
- [ ] 入口判定已显式声明走【学习】
- [ ] 波次 1：`01-项目概览.md` + `.codebase/scan-result.json` + `.codebase/scan-summary.md` 已写入
