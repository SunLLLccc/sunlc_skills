...
2. **按波次调度**：把子 skill（pm-scan / pm-techstack-generic / pm-conventions / pm-api-index / pm-build-deploy / pm-kb-index / pm-verify-lite（默认）+ pm-verify（opt-in））串成完整学习管线，按依赖关系分波次执行（波次 2 内部并行）。
...
- `{PROJECT_ROOT}/docs/project-knowledge/01-001-项目概览.md`（波次 1，pm-scan 产）
- `{PROJECT_ROOT}/.codebase/scan-result.json`（波次 1，pm-scan 产，机器契约）
- `{PROJECT_ROOT}/.codebase/scan-summary.md`（波次 1，pm-scan 产，人类速览）
- `{PROJECT_ROOT}/docs/project-knowledge/02-001-技术栈与架构.md`（波次 2，pm-techstack-generic 产）
- `{PROJECT_ROOT}/docs/project-knowledge/03-001-开发规范.md`（波次 2，pm-conventions 产）
- `{PROJECT_ROOT}/docs/project-knowledge/04-001-API索引.md`（波次 2，pm-api-index 产）
- `{PROJECT_ROOT}/docs/project-knowledge/05-001-构建打包部署.md`（波次 2，pm-build-deploy 产）
- `{PROJECT_ROOT}/docs/project-knowledge/README.md`（波次 3，pm-kb-index 产）
- `{PROJECT_ROOT}/docs/project-knowledge/06-001-校验报告.md`（波次 4，默认 `pm-verify-lite` 产；opt-in `pm-verify` 产全量版，文件名相同）
- `{PROJECT_ROOT}/docs/project-knowledge/_meta/manifest.json`（收尾，主会话写）
- `{PROJECT_ROOT}/docs/project-knowledge/_meta/progress.json`（全流程维护，断点续跑）
...
2. `ls {PROJECT_ROOT}/docs/project-knowledge/_meta/ 2>&1` —— 看 `_meta/` 下是否有 `manifest.json`（完成态记录）和 `progress.json`（在飞行态）；另 `ls {PROJECT_ROOT}/.codebase/scan-result.json 2>&1` 看扫描缓存是否存在（pm-scan 产，已取代 project-type.json）。
...
```
入口判定 →【学习】
  ├─ 波次 1：dispatch pm-scan        → 产 01 + .codebase/scan-result.json + scan-summary.md
  ├─ 波次 2：读 .codebase/scan-result.json，同一轮并行 dispatch 4 个 subagent
  │           ├─ pm-techstack-generic → 产 02
  │           ├─ pm-conventions       → 产 03
  │           ├─ pm-api-index         → 产 04
  │           └─ pm-build-deploy      → 产 05
  ├─ 波次 3：主会话执行 pm-kb-index   → 产 README
  ├─ 波次 4：dispatch pm-verify-lite（默认）/ pm-verify（opt-in）→ 产 06
  └─ 收尾：主会话写 manifest.json
```

**波次间的硬依赖**（不可乱序）：
- 波次 2 全部依赖波次 1 的 `01-001-项目概览.md`（作为上下文输入）和 `.codebase/scan-result.json`（路由依据）。
...
> 各 subagent 的精确 dispatch 指令模板详见 `references/wave-dispatch.md`（dispatch 时按需读取），此处不重复。

**前置依赖**：入口判定已声明走【学习】。
**本波次产出**：`01-001-项目概览.md` + `.codebase/scan-result.json` + `.codebase/scan-summary.md`。
**波次 1 完成标志**：上述三个文件都已写入，且 `scan-result.json` 的 `classifications` 有值（含恰好 1 条 `is_primary=true`）。

### 波次 2：读 scan-result.json 后，同一轮并行 dispatch 4 个分析 subagent

**执行方式**：**在同一轮 assistant 消息里**发出 4 个 Agent 工具调用（不要等一个回来再发下一个）。这是本子流程的并行收益点——4 个分析维度彼此正交，无共享上下文，可真正并发。

**波次 2 开始前的动作**（主会话执行）：
1. 读 `{PROJECT_ROOT}/.codebase/scan-result.json`，从 `classifications` 取 `is_primary=true` 的 type 作为路由依据（取代已退役 project-type.json 的 primaryType）。若多个 is_primary 或缺失，取 `confidence=confirmed` 且最核心的。
2. 读 `{PROJECT_ROOT}/docs/project-knowledge/01-001-项目概览.md`，确认上下文可用。
3. （可选）按 primary type 决定波次 2 是否用特化 skill——期 1 统一用 `pm-techstack-generic`（通用兜底）；期 2 实现特化后再按 primary type 路由。

> 各 subagent 的精确 dispatch 指令模板详见 `references/wave-dispatch.md`（dispatch 时按需读取），此处不重复。
**关于波次 2 内部依赖的说明**：pm-conventions / pm-api-index / pm-build-deploy 在各自 SKILL.md 里都声明"以 02-001-技术栈与架构.md 为上下文输入"。理论上它们应串行在 02 之后。但实践上：
...
**前置依赖**：波次 1 完成（01 + scan-result.json 已写入）。
...
3. 读 01-05 全部文档 + `.codebase/scan-result.json`，提炼"何时读"触发条件与一句话定位。
4. 写 `{PROJECT_ROOT}/docs/project-knowledge/README.md`，按 pm-kb-index 的模板（含项目一句话简介、文档地图、任务型阅读路径 ≥2 条、元信息章节）。
5. **注意**：此时 `_meta/manifest.json` 尚未写（收尾才写），README 的元信息章节里 `manifest.json` 相关字段标注"未生成（收尾步骤尚未执行）"或从 `.codebase/scan-result.json` 读已有字段，不编造。

**前置依赖**：波次 2 全部完成（02-05 已写入）。
**本波次产出**：`README.md`。
**波次 3 完成标志**：README.md 已写入，自检通过（项目一句话简介、每份文档"何时读"、≥2 条任务路径、元信息章节齐全）。

### 波次 4：dispatch pm-verify-lite（默认）/ pm-verify（opt-in）产 06 校验报告

**执行方式**：用 Agent 工具 dispatch 1 个 subagent。

**默认 dispatch `pm-verify-lite`**（轻量抽样校验：每文档 3-5 条、单次输出 ≤5000 字、只报告不修改）。**opt-in 升级到完整 `pm-verify`** 的触发条件（任一即用 full）：
- 人工指定跑 full；
- 关键项目（生产核心 / 高频 onboarding）；
- lite 报告风险信号为高（不一致项 ≥3 条，或关键文档 01/02/04 出现不一致）。

> 各 subagent 的精确 dispatch 指令模板（lite 默认 + full opt-in）详见 `references/wave-dispatch.md`（dispatch 时按需读取），此处不重复。

**前置依赖**：波次 1-3 全部完成（01-05 + README 已写入）。
**本波次产出**：`06-001-校验报告.md`（lite 与 full 同名，内容详略不同）。
**波次 4 完成标志**：06 已写入，自检通过（三元组、抽样范围声明、未修改任何 KB 文档）。
...
3. 读 `.codebase/scan-result.json` 的 `project.name` / `classifications`（取 is_primary）/ `generated_at` / `scanner.version`。
4. 读 06-001-校验报告的总体结论，提炼 verificationStatus。
5. `ls {PROJECT_ROOT}/docs/project-knowledge/` 确认全部文档生成状态。
6. 写 `{PROJECT_ROOT}/docs/project-knowledge/_meta/manifest.json`，按下述模板。

**manifest.json 模板**：

> 模板见 `templates/manifest.json`（收尾/断点续跑时读取填充）。
...
- [ ] 入口判定已显式声明走【学习】
- [ ] 波次 1：`01-001-项目概览.md` + `.codebase/scan-result.json` + `.codebase/scan-summary.md` 已写入
- [ ] 波次 2：`02` + `03` + `04` + `05` 四份文档已写入，且 4 个 dispatch 在同一轮并行
- [ ] 波次 3：`README.md` 已写入
- [ ] 波次 4：`06-001-校验报告.md` 已写入
- [ ] 收尾：`_meta/manifest.json` 已写入

## 断点续跑：_meta/progress.json（全流程维护）

pm 学习流程在 `{PROJECT_ROOT}/docs/project-knowledge/_meta/progress.json` 维护**在飞行态**，支持中断后续跑（小模型长程规划弱、易丢状态，这是"能跑完"的工程兜底）。**与 manifest.json 区分**：progress=在飞行态（每步更新），manifest=完成态记录（收尾一次性写）。

模板：

> 模板见 `templates/progress.json`（收尾/断点续跑时读取填充）。
...
4. **失败恢复**：`failed` 的 phase **不重跑全流程**——降级产出（标 uncertain/待补）+ 写 reason/partialOutput/retryable + 续跑下一独立 phase；retryable=true 的在收尾前重试一次。详细状态机与各 skill 失败兜底见 `references/failure-recovery.md`（按需读取）。
5. `generatedAt` 只在首次生成（progress.json 不存在）时写入；重跑不覆盖（禁止编造时间）。
6. `sourceCommit` 用 `git -C {PROJECT_ROOT} rev-parse --short HEAD`；无 git 标"未记录（无 git 仓库）"。
...
| 波次 4 pm-verify-lite（默认）/ pm-verify（opt-in） | subagent（dispatch） | 校验需读代码对照；默认 lite 轻量、full 全量 opt-in |
...
- 增量更新对应文档，而非全量重建（保留未变化文档的手工修订）——用 **AI/HUMAN 双区** + `scripts/update_ai_generated_block.py` 只替换 AI 区、保留人工补充（约定详见 `references/dual-zone-convention.md`，按需读取）。
- 重写 manifest.json 的 sourceCommit 为新 commit。

**期 2 实现时**：补充变化检测策略、增量波次调度规则、manifest diff 字段；双区脚本已就绪（P1-1）。
...
3. [ ] 若走【学习】：
   - [ ] **断点续跑**：开工写 `_meta/progress.json`（所有 phase 初始 pending）；若已存在则先读它，跳过 status=done 的 phase；每个 phase 完成后立即更新其 status（done/skipped/failed）+ output
   - [ ] **波次 1**：dispatch 1 个 pm-scan subagent（指令含 PROJECT_ROOT、遵循 SKILL.md、产出 01 + scan-result.json + scan-summary.md）
   - [ ] 确认波次 1 产出已写入（01 + scan-result.json + scan-summary.md）
   - [ ] **波次 2**：读 scan-result.json + 01，**在同一轮**并行 dispatch 4 个分析 subagent（techstack / conventions / api-index / build-deploy）
   - [ ] 确认波次 2 的 4 个产出已写入（02 + 03 + 04 + 05）
   - [ ] **波次 3**：主会话读 pm-kb-index SKILL.md，汇编 README
   - [ ] 确认 README 已写入
   - [ ] **波次 4**：dispatch 1 个 pm-verify-lite subagent（默认）；若触发 opt-in 条件（人工指定/关键项目/lite 报高风险）改 dispatch pm-verify
   - [ ] 确认 06 已写入
...
6. [ ] **自检**：
   ...
   - [ ] `_meta/progress.json` 是否全程维护？重跑能跳过 done 的 phase？路由是否改读 `.codebase/scan-result.json`（不再依赖 project-type.json）？
