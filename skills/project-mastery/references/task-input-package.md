...
| **previous-output.md** | 前置波次产出（01-项目概览 / 02-技术栈与架构） | dispatch 指令的"输入：上下文文档"段（如 01、02） |
...
- **无对应模板**（如 06-校验报告 lite/full 两版）：output-template 组件指明当前用哪个模板（lite 默认 / full opt-in）。
