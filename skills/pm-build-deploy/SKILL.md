...
- `01-001-项目概览.md`、`02-001-技术栈与架构.md`：波次 1/2 的产出（作为上下文输入，提供项目类型、目录结构、技术栈、多模块划分、自封装框架位置）

## 产出

- `{PROJECT_ROOT}/docs/project-knowledge/05-001-构建打包部署.md`
...
> 产出格式见 `templates/05-001-构建打包部署.md`（执行时读取该模板填充，勿自行发明结构）。

## 输入输出预算（小模型纪律）

- **单步单产物**：本次只产 `05-001-构建打包部署.md`（一篇），不并行产其它。
- **读预算**：第 0 步扫描所有构建配置（npm/Maven/Gradle/Python/Go/Rust/.../CI）识别多构建并存；配置文件 ≤20 个，单文件 ≤500 行；禁扫 node_modules/dist/target/build/.git/vendor/__pycache__；限深 ≤3 层。
- **写预算**：05 默认 8000-12000 字；探测清单逐项，**未发现的形态显式注明"未发现 {类型}"**，不臆造、不补写顾问建议。
- **证据先行**：每条命令/部署方式/版本号附【来源】+【标注】（已验证=配置字面量 / 推断=文档或间接推出 / 未发现）；无来源不入册。
