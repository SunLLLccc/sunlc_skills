...
1. `{PROJECT_ROOT}/docs/project-knowledge/01-001-项目概览.md` — 项目概览文档（人类可读）
...
**铁律**：机器字段一律用英文枚举（`confirmed`/`inferred`/`uncertain`）；人类文档（01-001-项目概览.md）一律用中文标签。`uncertain` 的结论必须同时在 `questions[]` 里写明缺口，不得臆造补全。
...
> 产出格式见 `templates/01-001-项目概览.md`（执行时读取该模板填充，勿自行发明结构）。
...
6. [ ] 计算置信度
7. [ ] 采集规模指标
8. [ ] 写入 `docs/project-knowledge/01-001-项目概览.md`（置信度用中文三态标签）
