...
├─ features/
│  ├─ inventory.md                ← 功能清单（lp-feature-scan）
│  └─ prompts/{功能}.md           ← 教学提示词（lp-prompt-gen）
└─ docs/{功能}.md                 ← 学习文档（doc-gen subagent）
```
...
1. 探测 `{PROJECT_ROOT}/docs/project-knowledge/01-项目概览.md` 与 `02-技术栈与架构.md` 是否存在。
2. **存在** → 直接读，作为后续步骤与学习者的背景，跳过重扫。
3. **不存在** → dispatch `pm-scan` 产出 01 + `_meta/project-type.json`，再 dispatch `pm-techstack-generic` 产出 02。产出落 project-knowledge/（A 的目录），作为 B 的背景输入。

### 第 1 步：功能扫描

- dispatch `lp-feature-scan` → 产出 `features/inventory.md`（功能清单，每条带 `- [ ]` 勾选框、核心度/复杂度/依赖/证据）。

### 第 2 步：人在回路挑选（主会话）

- 把 inventory.md 呈现给用户挑选要深入的功能：
  - 若核心功能 ≤4 个，可用 AskUserQuestion（multiSelect）让用户勾选。
  - 若更多，请用户直接编辑 inventory.md 把 `- [ ]` 改为 `- [x]`，或口头告知功能名/编号。
...
1. dispatch `lp-prompt-gen`（输入：该功能在 inventory 的证据 + 01/02 背景）→ 产出 `features/prompts/{功能}.md`。
2. 用该提示词 dispatch 一个**通用 doc 生成 subagent**：
   - 给 subagent 的指令 = "读取提示词文件 → 按其必读证据实际打开代码阅读 → 按其 6 节结构与撰写要求产出文档 → 写到 docs/{功能}.md"。
...
3. [ ] 第 1 步：dispatch lp-feature-scan → inventory.md
