...
- `01-001-项目概览.md`：波次 1 pm-scan 的产出（作为上下文输入，提供项目类型、目录结构、入口点、顶层技术栈速览）
- `.codebase/scan-result.json`：波次 1 pm-scan 的机器契约缓存（Step 0 优先读）

## 产出

- `{PROJECT_ROOT}/docs/project-knowledge/02-001-技术栈与架构.md`
...
> 产出格式见 `templates/02-001-技术栈与架构.md`（执行时读取该模板填充，勿自行发明结构）。

## 输入输出预算（小模型纪律）

- **单步单产物**：本次只产 `02-001-技术栈与架构.md`（一篇），不并行产其它。
